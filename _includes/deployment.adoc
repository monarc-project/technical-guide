...
* Download the link:https://github.com/monarc-project/MonarcAppFO/releases/latest[image] from the last release of MONARC (file with the `*.ova` extension).
...
* Start the VM and login in the CLI interface. Login and password are provided on the GitHub link:https://github.com/monarc-project/MonarcAppFO/releases/latest[release page],
section `SSH login (Ubuntu credentials)` (usually login: `monarc`, password: `password`).
* Execute `ip address` command in the CLI and find ip v4 address. Should be something like `192.168.0.123`, where the last 3 digits are different.

image:monarc_vm_ip_address.png[MONARC Architecture, align="center", scaledwidth="50%"]

* Use the ip in your browser to launch the application (eg. http://192.168.0.188). Recommended to use Mozilla Firefox or Google Chrome.
* Login to your application. the credentials are provided on the GitHub link:https://github.com/monarc-project/MonarcAppFO/releases/latest[release page],
